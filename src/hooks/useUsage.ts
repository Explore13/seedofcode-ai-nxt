 ...
 */
async function fetchRowsSince(earliest: Date): Promise<UsageLog[]> {
  const rows: UsageLog[] = [];
  const boundary = earliest.getTime();

  for (let page = 1; page <= MAX_PAGES; page++) {
    const { data, meta } = await usageApi.list({ page, limit: PAGE_LIMIT });
    rows.push(...data);
 ...
 */
export function useUsage(from: Date, to: Date) {
  const rangeMs = Math.max(to.getTime() - from.getTime(), 0);
  const prevFrom = new Date(from.getTime() - rangeMs);

  return useQuery({
    queryKey: ['usage', 'range', from.toISOString(), to.toISOString()],
    queryFn: () => fetchRowsSince(prevFrom),
  });
}
